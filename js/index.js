...
(function() {
  $(document).ready(function() {
    var Filter, SavedInput, Video, VideoList, autoplayInput, expandInput, filter, historyInput, onPlayerReady, onPlayerStateChange, readDataInterval, title, unwatchedVideos, updateInput, watchedVideos;
    Video = (function() {
    ...
    VideoList = (function() {
      function VideoList(storageString, selector, filterClass1, reversed) {
        var video, videoList;
    ...
    Filter = (function() {
      function Filter(storageString, elementSelector) {
        ...
        filterClass = this;
        addRow = function(channel, type, regex) {
          var row;
          row = $('<div/>', {
            "class": 'row'
          });
        ...
        this.element.change(function() {
          ...
          filterClass.element.children().each(function() {
            return filterClass.contents.push({
              ...
              regex: $(this).children('.regex').val()
            });
          });
          return filterClass.save();
        });
      }
      ...
      Filter.prototype.allows = function(video) {
        ...
        for (j = 0, len = ref.length; j < len; j++) {
          ...
          if (video.author === filter.channel) {
            if (filter.type === 'blacklist' && video.title.match(filter.regex)) {
              return false;
            }
            if (filter.type === 'whitelist' && !video.title.match(filter.regex)) {
              return false;
            }
          }
        }
